...
This data is from the Community Resistance in Athens Project, a research project funded
by the CDC and conducted through the University of Georgia. 
This data is a combination of results from an epidemiological survey and resistance patterns
from the E. coli of the participants' stool samples.
...
* The raw data for the Community Resistance in Athens Project is located in the `raw_data` folder.
...
* The `products` folder will contain an example `bibtex` and CSL style file for references. Those files are used by the example manuscript, poster and slides.
* The `poster` and `slides` folders will contain very basic examples of posters and slides made with R Markdown. Note that especially for slides, there are many different formats. You might find a different format more suitable. Check the R Markdown documentation. 
...
# Getting started
Look at the processing_code and run the code so it is clean. This will produce the clean
dataset.
